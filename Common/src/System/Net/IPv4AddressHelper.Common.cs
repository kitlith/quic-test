...
namespace System.Net
{
    internal static partial class IPv4AddressHelper
    {
        ...
        internal static bool IsValid(ReadOnlySpan<char> name, out int end, bool allowIPv6, bool notImplicitFile, bool unknownScheme)
        {
        ...
        // again, for byte
        internal static bool IsValid(ReadOnlySpan<byte> name, int start, ref int end, bool allowIPv6, bool notImplicitFile, bool unknownScheme)
        {
            // IPv6 can only have canonical IPv4 embedded. Unknown schemes will not attempt parsing of non-canonical IPv4 addresses.
            if (allowIPv6 || unknownScheme)
            {
        ...
        // again, for byte
        internal static bool IsValidCanonical(ReadOnlySpan<byte> name, out int end, bool allowIPv6, bool notImplicitFile)
        {
            end = 0; // Default value in case of failure
            int dots = 0;
            long number = 0;
            ...
            while (start < name.Length)
            {
                ...
                if (parsedCharacter < IPv4AddressHelper.Decimal)
                {
                    ...
                    if (!haveNumber && parsedCharacter == 0)
                    {
                        if ((start + 1 < name.Length) && name[start + 1] == (byte)'0')
                        {
...
>>>>>>> Conflict 1 of 1 ends
        ...
        internal static long ParseNonCanonical(ReadOnlySpan<char> name, out int end, bool notImplicitFile)
        {
            end = 0; // Default value in case of failure
            int numberBase;
            int ch = 0;
        ...
        // again, for byte
        internal static long ParseNonCanonical(ReadOnlySpan<byte> name, out int end, bool notImplicitFile)
        {
            end = 0; // Default value in case of failure
            int numberBase;
            int ch = 0;
